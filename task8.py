...
if __name__ == '__main__':
    ...
    learning_rate = 0.05  # Adjusted learning rate for faster convergence

    # Training with CrossEntropyLoss
    model = mobilenet_v3_small(weights=None, num_classes=2).to(device)
    criterion_ce = torch.nn.CrossEntropyLoss()
    print("Training with CrossEntropyLoss:")
    train_and_evaluate(model, criterion_ce, trainloader, testloader, device, "CrossEntropy", learning_rate, batch_size=1024)

    # Reset model for NLLLoss
    model = mobilenet_v3_small(weights=None, num_classes=2).to(device)
    criterion_nll = torch.nn.NLLLoss()
    print("Training with NLLLoss:")
    train_and_evaluate(model, criterion_nll, trainloader, testloader, device, "NLLLoss", learning_rate, batch_size=1024)